...
function builder(table){
 ...
 async function execute(){
  try{
   ...
   // Firestore Rules are user-isolated by user_id. Every collection query
   // must therefore explicitly constrain the query to the signed-in user.
   if(user?.uid && table !== "users" && !filters.some(([f])=>f === "user_id")) {
     clauses.unshift(where("user_id","==",user.uid));
   }
...
async function saveCategory(e){
  e.preventDefault();
  msg("catMsg","");
  if(!user?.uid) return msg("catMsg","Faça login novamente antes de cadastrar a categoria.");
  const name=$("catName").value.trim();
  const type=$("catType").value;
  if(!name) return msg("catMsg","Informe o nome da categoria.");
  if(!["entrada","saida","ambos"].includes(type)) return msg("catMsg","Tipo de categoria inválido.");
  if(categories.some(c=>(c.name||"").trim().toLowerCase()===name.toLowerCase()&&c.type===type)) return msg("catMsg","Essa categoria já existe.");
  try{
    const ref=doc(collectionRef("categories"));
    await setDoc(ref,{user_id:user.uid,name,type,created_at:serverTimestamp()});
    ...
    msg("catMsg",friendlyError(error));
  }
}
...
async function saveTx(e){e.preventDefault();let n=+$("txInstall").value||1,g=crypto.randomUUID(),base={user_id:user.uid,type:$("txType").value,amount:+$("txAmount").value,transaction_date:$("txDate").value,category_id:$("txCat").value,account_id:$("txAccount").value||null,card_id:$("txCard").value||null,status:$("txStatus").value,description:$("txDesc").value,notes:$("txNotes").value||null,group_id:g};let rows=[];for(let i=0;i<n;i++){let d=new Date($("txDate").value+"T12:00:00");d.setMonth(d.getMonth()+i);rows.push({...base,transaction_date:d.toISOString().slice(0,10),installment_number:i+1,installment_total:n})}let r=await sb.from("transactions").insert(rows);msg("txMsg",r.error?.message||"Lançamento salvo.");if(!r.error){$("txForm").reset();$("txDate").value=today;await load()}}
async function saveCard(e){e.preventDefault();let r=await sb.from("cards").insert({user_id:user.uid,name:$("cardName").value,limit_amount:+$("cardLimit").value,closing_day:+$("cardClose").value,due_day:+$("cardDue").value});msg("cardMsg",r.error?.message||"Cartão cadastrado.");if(!r.error){$("cardForm").reset();await load()}}
async function saveRec(e){e.preventDefault();let r=await sb.from("recurring").insert({user_id:user.uid,description:$("recDesc").value,amount:+$("recAmount").value,category_id:$("recCat").value,due_day:+$("recDay").value,start_date:$("recStart").value,end_date:$("recEnd").value||null});msg("recMsg",r.error?.message||"Cadastrado.");if(!r.error){$("recForm").reset();await load()}}
