...
function toggleSplitPayment(){const box=$("splitPaymentBox"),enabled=$("txType")?.value==="entrada"&&$("txSplitEnabled")?.checked;if(box)box.classList.toggle("hidden",!enabled);fillPaymentCardSelects();}
function fillPaymentCardSelects(){const opts='<option value="">Cartão</option>'+cards.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join("");["txPay1Card","txPay2Card"].forEach(id=>{const el=$(id);if(el){const old=el.value;el.innerHTML=opts;el.value=old;}})}
...
function getPaymentParts(){const enabled=$("txType")?.value==="entrada"&&$("txSplitEnabled")?.checked;if(!enabled)return null;return [1,2].map(i=>({method:$(`txPay${i}Method`)?.value,amount:Number($(`txPay${i}Amount`)?.value||0),installments:Math.max(1,Number($(`txPay${i}Installments`)?.value||1)),card_id:$(`txPay${i}Card`)?.value||null})).filter(x=>x.amount>0);}
function paymentSummary(t){if(Array.isArray(t.payment_parts)&&t.payment_parts.length)return t.payment_parts.map(p=>`${paymentLabel(p.method)} ${money(p.amount)}${p.method==='credit'&&Number(p.installments)>1?` em ${p.installments}x`:''}`).join(' + ');return paymentLabel(t.payment_method)}
async function saveTx(e){e.preventDefault();let n=+$('txInstall').value||1,g=crypto.randomUUID();const pedido=isPedidoCategory();let attachmentUrl=null;const file=$('txAttachmentFile')?.files?.[0];if(file){try{const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,"_");const rr=storageRef(storage,`users/${user.uid}/comprovantes/${g}-${safe}`);await uploadBytes(rr,file);attachmentUrl=await getDownloadURL(rr)}catch(err){return msg("txMsg","Não foi possível enviar o comprovante: "+friendlyError(err))}}const isEntrada=$("txType").value==="entrada";const paymentParts=getPaymentParts();if(isEntrada&&$("txSplitEnabled")?.checked){const total=Number($("txAmount").value||0),sum=(paymentParts||[]).reduce((s,p)=>s+p.amount,0);if(!paymentParts?.length)return msg("txMsg","Informe pelo menos uma forma de recebimento.");if(Math.abs(sum-total)>0.01)return msg("txMsg",`As formas de recebimento somam ${money(sum)}, mas o lançamento é ${money(total)}.`);}let method=$("txMethod").value;if(paymentParts?.length)method=paymentParts.length>1?'multiple':paymentParts[0].method;let base={user_id:user.uid,type:$("txType").value,amount:+$("txAmount").value,transaction_date:$("txDate").value,competence_date:$("txDate").value,paid_date:$("txPaidDate").value||null,category_id:$("txCat").value,subcategory:$("txSubcategory").value||null,account_id:$("txAccount").value||null,card_id:$("txCard").value||null,payment_method:method,status:$("txStatus").value,name:$("txName").value.trim(),description:$("txDesc").value.trim()||null,notes:$("txNotes").value||null,attachment_url:attachmentUrl,recurrence:$("txRecurring").value,group_id:g,dre_class:$("txDreClass")?.value||(isEntrada?'receita_bruta':'despesa_operacional')};if(paymentParts?.length)base.payment_parts=paymentParts;if(pedido){base.metal_value=$("txMetalValue").value===""?null:+$("txMetalValue").value;base.initial_kg=$("txInitialKg").value===""?null:+$("txInitialKg").value;base.final_kg=$("txFinalKg").value===""?null:+$("txFinalKg").value;}let rows=[];for(let i=0;i<n;i++){let d=new Date($("txDate").value+"T12:00:00");d.setMonth(d.getMonth()+i);let pd=base.paid_date;if(pd&&i){let x=new Date(pd+"T12:00:00");x.setMonth(x.getMonth()+i);pd=x.toISOString().slice(0,10)}rows.push({...base,transaction_date:d.toISOString().slice(0,10),competence_date:d.toISOString().slice(0,10),paid_date:pd,installment_number:i+1,installment_total:n})}let r=await sb.from("transactions").insert(rows);msg("txMsg",r.error?.message||"Lançamento salvo.");if(!r.error){$("txForm").reset();$("txDate").value=today;$("txPaidDate").value="";togglePedidoFields();toggleSplitPayment();toggleDreClass();await load()}}
